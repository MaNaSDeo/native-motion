...
const ITEM_MARGIN = 8;
const ITEM_WIDTH =
  width / NUM_COLUMNS - (ITEM_MARGIN * (NUM_COLUMNS + 1)) / NUM_COLUMNS;
...
export default function AnimatedHeaderDemo() {
  ...
  const renderItem = ({ item }: { item: { id: string } }) => {
    return (
      <View
        key={item.id}
        style={[styles.cardItem, { backgroundColor: colors.Neutral50 }]}
      >
        {/* Skeleton Placeholders */}
        <View
          style={[styles.skeletonImage, { backgroundColor: colors.Neutral90 }]}
        />
        <View
          style={[
            styles.skeletonLine,
            { width: "100%", backgroundColor: colors.Neutral90 },
          ]}
        />
        <View
          style={[
            styles.skeletonLine,
            { width: "80%", backgroundColor: colors.Neutral90 },
          ]}
        />
        <View
          style={[
            styles.skeletonLine,
            { width: "60%", backgroundColor: colors.Neutral90 },
          ]}
        />
      </View>
    );
  ...
  return (
    <SafeAreaView
      style={[styles.container, { backgroundColor: colors.Neutral0 }]}
    >
